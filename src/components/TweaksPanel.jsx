// ─── TweaksPanel ──────────────────────────────────────────────────────────────
// Panel flotante de ajustes en vivo: colores, tipografía, marquesina, etc.
// Actívalo con el botón "⚙ Tweaks" en la esquina inferior derecha.
...
(function () {
  ...
  function TweakToggle({ label, value, onChange }) {
    return (
      <div className="twk-row" style={{ flexDirection: "row", alignItems: "center", justifyContent: "space-between" }}>
    ...
    return (
      <TweakRow label={label}>
        <div className="twk-chips" role="radiogroup">
          {options.map((o, i) => {
            const [hero] = Array.isArray(o) ? o : [o];
            const on = key(o) === cur;
            return (
              <button key={i} type="button" className="twk-chip" role="radio"
                data-on={on ? "1" : "0"} style={{ background: hero }}
                onClick={() => onChange(o)}>
                {on && (
  ...
  /* ── Logo swatches (control personalizado) ────────────────────────────────── */
  function LogoSwatches({ value, onChange }) {
    const opts = [
      { id: "sticker", label: "Sticker", src: window.ASSET.mascotSticker },
      { id: "blanco",  label: "Blanco",  src: window.ASSET.mascotWhite   },
      { id: "crema",   label: "Crema",   src: window.ASSET.mascotCream   },
    ];
    return (
      <div style={{ display: "flex", gap: 8, paddingBottom: 6 }}>
        {opts.map((o) => (
          <button key={o.id} onClick={() => onChange(o.id)}
            style={{
              flex: 1, cursor: "pointer", borderRadius: 12, padding: "8px 4px 6px",
              border: value === o.id ? "2px solid #0F0D0D" : "2px solid transparent",
              background: value === o.id ? "rgba(196,30,30,0.08)" : "transparent",
              display: "flex", flexDirection: "column", alignItems: "center", gap: 5,
            }}>
            <span style={{ width: 46, height: 46, borderRadius: "50%", background: "#C41E1E",
              display: "grid", placeItems: "center", border: "2px solid #0F0D0D" }}>
              <img src={o.src} alt={o.label} style={{ width: "78%", height: "78%", objectFit: "contain" }} />
            </span>
            <span style={{ fontSize: 11, fontWeight: 700, color: "#0F0D0D" }}>{o.label}</span>
          </button>
        ))}
      </div>
    );
  ...
  function TweaksPanel({ title = "Tweaks", children }) {
    ...
    const offsetRef = useRef({ x: 100, y: 16 });
    const PAD = 16;
  ...
  /* Exporta todo al scope global */
  Object.assign(window, {
    TweaksPanel, TweakSection, TweakRow,
    TweakSlider, TweakToggle, TweakRadio, TweakColor, TweakText, LogoSwatches,
  });
})();
